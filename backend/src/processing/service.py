...
from src.ocr.schemas import OCRItem, OCRReceiptData
...
class BillsProcessorService:
    ...
    async def process_receipt(self, bill_id: int) -> None:
        ...
        try:
            ...
            if not await self._try_acquire_processing_lock(bill_id):
                ...
                logger.info(
                    f"Bill {bill_id} is already being processed by another process, skipping"
                )
                return
            ...
            shop_id = await self._get_or_create_shop(
                ocr_data.shop_name, ocr_data.shop_address
            )
            ...
            for idx, normalized_item in enumerate(normalized_items, start=1):
                unit_price_str = (
                    f"{normalized_item.unit_price:.2f}"
                    if normalized_item.unit_price
                    else "N/A"
                )
                logger.info(
            ...
            logger.error(
                f"Error processing receipt bill_id={bill_id}: {e}", exc_info=True
            )
            ...
            try:
                ...
                logger.critical(
                    f"CRITICAL: Failed to save error status for bill {bill_id}: {inner_e}",
                    exc_info=True,
                )
    ...
    async def _extract_receipt_data(
        ...
        upload_file = UploadFile(
            file=file_obj, filename=filename, headers={"content-type": mime_type}
        )
    ...
    async def _get_or_create_shop(
        self, shop_name: str | None, shop_address: str | None
    ) -> int | None:
        ...
        try:
            shop = await self.shop_service.get_or_create_by_name(
                name=shop_name, address=shop_address
            )
    ...
    async def _normalize_items(
        ...
        for ocr_item in ocr_items:
            try:
                ...
                logger.error(
                    f"Failed to normalize item '{ocr_item.name}': {e}", exc_info=True
                )
    ...
    async def _create_bill_items(
        self, bill_id: int, items: list[NormalizedItem]
    ) -> None:
        """
        ...
        for normalized_item in items:
            try:
                ...
                bill_item_data = self._map_normalized_to_bill_item(
                    bill_id=bill_id, normalized_item=normalized_item
                )
        ...
        logger.info(
            f"Created {created_count}/{len(items)} bill_items for bill_id={bill_id}"
        )

    def _map_normalized_to_bill_item(
        self, bill_id: int, normalized_item: NormalizedItem
    ) -> BillItemCreate:
        """
        ...
        needs_verification = (
            has_negative_price
            or not normalized_item.is_confident
            or normalized_item.confidence_score < 0.8
        )
    ...
    async def _set_error(self, bill_id: int, error_message: str) -> None:
        ...
        update_data = BillUpdate(
            status=ProcessingStatus.ERROR, error_message=truncated_error
        )
