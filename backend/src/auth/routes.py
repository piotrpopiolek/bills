...
async def verify_magic_link(
    ...
    token: str = Query(..., description="Magic link token from URL"),
) -> TokenResponse:
    """
...
async def refresh_tokens(
    data: TokenRefreshRequest, service: ServiceDependency
) -> TokenResponse:
    """
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    summary="List all magic links (Admin)",
    description="Get paginated list of all magic links. For admin/debugging purposes only.",
    tags=["admin"],
)
...
@router.get(
    "/admin/magic-links/{magic_link_id}",
    response_model=MagicLinkInternalResponse,
    status_code=status.HTTP_200_OK,
    summary="Get magic link by ID (Admin)",
    description="Get detailed information about a specific magic link. For admin/debugging purposes only.",
    tags=["admin"],
)
async def get_magic_link(
    magic_link_id: int, user: CurrentUser, service: ServiceDependency
) -> MagicLinkInternalResponse:
    """
...
@router.patch(
    ...
    response_model=MagicLinkInternalResponse,
    status_code=status.HTTP_200_OK,
    summary="Update magic link (Admin)",
    description="Update magic link fields. For admin/debugging purposes only.",
    tags=["admin"],
)
async def update_magic_link(
    magic_link_id: int,
    data: MagicLinkUpdate,
    user: CurrentUser,
    service: ServiceDependency,
):
    """
...
@router.delete(
    ...
    summary="Delete magic link (Admin)",
    description="Delete a magic link record. For admin/debugging purposes only.",
    tags=["admin"],
)
