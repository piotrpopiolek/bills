...
from src.telegram.context import (
    ...
    get_storage_service_for_telegram,
    get_user,
)
...
async def login_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        logger.error(
            f"User not found in context for telegram_id {update.effective_user.id}"
        )
    ...
    async with get_or_create_session() as session:
        ...
        try:
            ...
            await update.message.reply_text(
                "Użytkownik nie został znaleziony. Spróbuj /start."
            )
...
async def daily_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_or_create_session() as session:
        try:
            ...
            logger.error(
                f"Error generating daily report for user {user_id}: {e}", exc_info=True
            )
...
async def weekly_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_or_create_session() as session:
        try:
            ...
            logger.error(
                f"Error generating weekly report for user {user_id}: {e}", exc_info=True
            )
            await update.message.reply_text(get_user_message(e))
...
async def monthly_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_or_create_session() as session:
        try:
            ...
            logger.error(
                f"Error generating monthly report for user {user_id}: {e}",
                exc_info=True,
            )
            await update.message.reply_text(get_user_message(e))
...
async def verify_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bill_id <= 0:
        await update.message.reply_text("⚠️ ID rachunku musi być liczbą dodatnią.")
        return
    ...
    async with get_or_create_session() as session:
        try:
            ...
            if (
                bill.status == ProcessingStatus.PENDING
                or bill.status == ProcessingStatus.PROCESSING
            ):
                await update.message.reply_text(
            ...
            logger.error(
                f"Error in verify_command for bill_id={bill_id}: {e}", exc_info=True
            )
            await update.message.reply_text(
                "⚠️ Wystąpił błąd podczas rozpoczynania weryfikacji.\n"
                "Spróbuj ponownie później lub skontaktuj się z supportem."
            )
...
async def handle_receipt_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_or_create_session() as session:
        ...
        if remaining_bills <= 0:
            ...
            logger.info(
                f"User {user_id} exceeded monthly limit: {bills_this_month}/{monthly_limit}"
            )
            return
        ...
        try:
            ...
            image_url, image_hash = await storage_service.upload_file_content(
                file_content=bytes(file_content), user_id=user_id, extension=extension
            )

            # Check for duplicate bills with same image_hash
            stmt = (
                select(Bill)
                .where(Bill.image_hash == image_hash)
            ...
            if existing_bills:
                ...
                if existing_bill.status in (
                    ...
                    items_count = (
                        len(bill_with_items.bill_items)
                        if bill_with_items.bill_items
                        else 0
                    )
                    status_text = (
                        "✅ Paragon przetworzony!"
                        if bill_with_items.status == ProcessingStatus.COMPLETED
                        else "✅ Paragon przetworzony!"
                    )
                    verification_text = (
                        "\n⚠️ Niektóre pozycje wymagają weryfikacji."
                        if bill_with_items.status == ProcessingStatus.TO_VERIFY
                        else ""
                    )
            ...
            try:
                ...
                if updated_bill.status == ProcessingStatus.COMPLETED:
                    items_count = (
                        len(updated_bill.bill_items) if updated_bill.bill_items else 0
                    )
                    await status_message.edit_text(
                    ...
                    error_msg = (
                        updated_bill.error_message[:100]
                        if updated_bill.error_message
                        else "Nieznany błąd"
                    )
                    await status_message.edit_text(
                    ...
                    unverified_count = sum(
                        1 for item in updated_bill.bill_items if not item.is_verified
                    )
                ...
                logger.error(
                    f"Error processing receipt bill_id={bill.id}: {e}", exc_info=True
                )
            ...
            logger.error(
                f"Resource not found during receipt processing: {e}", exc_info=True
            )
            await status_message.edit_text(
                "Nie znaleziono wymaganego zasobu. Spróbuj ponownie."
            )
        except Exception as e:
...
async def start_bill_verification(
    update: Update, context: ContextTypes.DEFAULT_TYPE, bill_id: int, user_id: int
):
    """
    ...
    async with get_or_create_session() as session:
        try:
            ...
            message_text = format_bill_item_for_verification(
                item=item_with_relations, item_number=1, total_items=total_items
            )
            ...
            logger.error(
                f"Error starting bill verification bill_id={bill_id}: {e}",
                exc_info=True,
            )
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
...
async def handle_item_verification_callback(
    ...
    try:
        ...
        await query.edit_message_text(
            "⚠️ Nieprawidłowy format danych. Spróbuj ponownie."
        )
        return
        ...
        try:
            ...
            if action == "approve":
                ...
                await verification_service.verify_item(
                    bill_item_id=bill_item_id, user_id=user_id
                )
            ...
            next_item = await verification_service.get_next_unverified_item(
                bill_id=bill_id, user_id=user_id, exclude_item_ids=None
            )
            ...
            if next_item:
                ...
                if await verification_service.check_all_items_verified(
                    bill_id, user_id
                ):
            ...
            await query.edit_message_text(
                "⚠️ Wystąpił błąd podczas przetwarzania. Spróbuj ponownie."
            )


async def handle_item_edit_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ...
    if not edited_text:
        await update.message.reply_text(
            "⚠️ Tekst nie może być pusty. Wpisz nazwę produktu lub /cancel aby anulować."
        )
        return
    ...
    async with get_or_create_session() as session:
        try:
            ...
            await verification_service.verify_item(
                bill_item_id=editing_item_id, user_id=user_id, edited_text=edited_text
            )
            ...
            if bill_id:
                ...
                if next_item:
                    ...
                    if await verification_service.check_all_items_verified(
                        ...
                        # Finalizuj weryfikację
                        await verification_service.finalize_verification(
                            bill_id, user_id
                        )
            ...
            await update.message.reply_text(
                "⚠️ Wystąpił błąd podczas aktualizacji pozycji. Spróbuj ponownie."
            )
