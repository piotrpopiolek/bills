...
class OCRService:
    ...
    async def extract_data(self, file: UploadFile) -> OCRReceiptData:
        ...
        try:
            ...
            logger.info(
                "Gemini response parsed successfully", extra={"response": result}
            )
        except Exception as e:
            logger.error(
            ...
            raise ExtractionError(
                f"Nie udało się przetworzyć odpowiedzi z Gemini: {str(e)}"
            ) from e
    ...
    async def _prepare_image_part(
        self, file: UploadFile, mime_type: str
    ) -> dict[str, Any]:
        """
...
"""
    ...
    async def _call_gemini_with_retry(self, parts: list[Any]) -> LLMReceiptExtraction:
        ...
        try:
            ...
            logger.error(
                f"Failed to parse Gemini JSON response: {str(e)}", exc_info=True
            )
            raise AIServiceError(
                f"Nieprawidłowa odpowiedź JSON z Gemini API: {str(e)}"
            ) from e
        except Exception:
        ...
        for pattern, format_name in date_only_patterns:
            ...
            if match:
                try:
                    ...
                    logger.debug(
                        f"Failed to parse date-only with pattern {pattern}: {e}"
                    )
                    continue
    ...
    def _parse_response(self, llm_response: LLMReceiptExtraction) -> OCRReceiptData:
        ...
        if llm_response.date:
            ...
            if date is None:
                logger.warning(
                    f"Failed to parse date from LLM response: {llm_response.date}"
                )
