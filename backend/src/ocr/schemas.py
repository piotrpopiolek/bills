...
class OCRItem(AppBaseModel):
    ...
    unit_price: Decimal | None = Field(None, description="Cena jednostkowa")
    total_price: Decimal = Field(..., description="Cena całkowita pozycji")
    category_suggestion: str | None = Field(
        None, max_length=100, description="Sugerowana kategoria produktu (z LLM)"
    )
    confidence_score: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Ocena pewności odczytu (0.0-1.0)"
    )
...
class OCRReceiptData(AppBaseModel):
    ...
    shop_name: str | None = Field(None, max_length=200, description="Nazwa sklepu")
    shop_address: str | None = Field(None, max_length=500, description="Adres sklepu")
    date: datetime | None = Field(None, description="Data i czas zakupu")
    ...
    requires_verification: bool = Field(
        default=False, description="Flaga wymaga weryfikacji"
    )
...
class OCRExtractResponse(AppBaseModel):
    ...
    message: str = Field(
        default="Ekstrakcja zakończona pomyślnie",
        description="Komunikat dla użytkownika",
    )
    data: OCRReceiptData = Field(..., description="Wyekstrahowane dane")
    raw_text: str | None = Field(
        None, description="Surowy tekst z OCR (opcjonalnie, do debugowania)"
    )
...
class LLMReceiptItem(AppBaseModel):
    ...
    model_config = ConfigDict(
        strict=True, extra="forbid"  # Wymuszenie typów  # Odrzucenie nieznanych pól
    )
...
class LLMReceiptExtraction(AppBaseModel):
    ...
    date: str | None = (
        None  # Date in ISO 8601 (preferred) or formats like DD/MM/YYYY HH:MM, DD.MM.YYYY HH:MM
    )
    total_amount: float
